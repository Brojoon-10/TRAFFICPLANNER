...
# <--- 변경된 부분: 새로운 GNN-Transformer 모델을 import 합니다.
from models.fit_traffic_model_trans import FITTrafficModel
# <--- (기존) from models.fit_traffic_model import FITTrafficModel
...
#------------------------------Loss Added (사용자 코드 유지)------------------------------    
    ...
    for epoch in range(ckpt_epoch, cfg.epochs):
        ...
        if epoch % cfg.val_every == 0:
            ...
            with torch.no_grad():
                ...
                if mean_eval_loss < min_eval_loss:
                    ...
                    save_file = os.path.join(ckpts_path, 'best_eval_model_trans_500_with_bicycle.pth')
        ...
        if epoch % cfg.save_every == 0:
            ...
            save_state(save_file, model, optimizer, cur_epoch=epoch, min_val_loss=min_eval_loss)
            save_file = os.path.join(ckpts_path, 'latest_model_trans_500_with_bicycle.pth')
            save_state(save_file, model, optimizer, cur_epoch=epoch, min_val_loss=min_eval_loss)
            if use_wandb:
                wandb.save(save_file)
