...
import datasets.fit_utils as futils
...
class FITMapEnv(object):
    def __init__(self, map_data_path,
        ...
        '''
        super(FITMapEnv, self).__init__()

        self.data_path = map_data_path
        self.nusc_maps = futils.get_fit_maps(map_data_path)
        if load_lanegraph:
            print('Loading lane graphs...')
            ############### MODIFIED FOR FIT/LANELET2 DATASET ###############
            ...
            # Use Lanelet2-specific processing:
            # self.lane_graphs = {map_name: futils.process_lanegraph_lanelet2(nmap, lanegraph_res_meters, lanegraph_eps) for map_name,nmap in self.nusc_maps.items()}
            #################################################################
        # self.map_list = list(self.nusc_maps.keys())
        # self.map_list = ["HMCL_RACING"]
        self.map_list = ["boston_seaport"]
        ...
        self.road_list = ['drivable_area']
        road_layers = [lay for lay in self.layer_names if lay in self.road_list]
        nonroad_layers = [lay for lay in self.layer_names if lay not in self.road_list]
        self.layer_map = {}
        
        # 'drivable_area' will be assigned to channel 0.
        for lay in road_layers:
            self.layer_map[lay] = 0
        # 'solid_line' and 'dashed_line' will be assigned to channels 1 and 2.
        lay_idx = 1
        for lay in nonroad_layers:
            self.layer_map[lay] = lay_idx
            lay_idx += 1
        
        self.num_layers = len(self.layer_map)
        print(f"Map environment initialized with {self.num_layers} layers: {self.layer_map}")
        
        print('Rasterizing custom maps...')

        self.nusc_dx = np.array([0.25, 0.25])

        # Map files are stored in src/maps/
        maps_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'maps')
        drivable_area_osm = os.path.join(maps_dir, 'centerline_added_boston.osm')
        line_markings_osm = os.path.join(maps_dir, 'boston_raster_map_for_potential_loss.osm')
        shp_path = os.path.join(maps_dir, 'shp')

        self.nusc_raster = futils.fit_raster(drivable_area_osm, line_markings_osm, shp_path)
        
               # print("!!!! raster !!!! :")
        # print(futils.fit_raster(lanelet_path, shp_path))
    ...
    def get_map_crop(self, scene_graph, map_idx,
        ...
        # print("map_obs:", torch.sum(map_obs))
        return map_obs
