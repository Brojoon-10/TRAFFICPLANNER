...
def parse_cfg():
    ...
    # <--- 변경된 부분: GNN-Transformer 모델 파라미터 추가
    # --- (기존 past_feat_size, future_feat_size 제거) ---
...
def run_one_epoch(data_loader, model, map_env, loss_fn, device, out_path,
    ...
    (이 함수는 모델의 forward 시그니처가 호환되므로 수정이 필요 없습니다.)
    '''
    if use_wandb:
        import wandb
    ...
    for i, data in enumerate(pbar):
        ...
        try: 
            ...
            if not train: 
                ...
                # Retrieve new loss values, providing a default of 0.0 if they don't exist.
                accel_val = loss_dict.get('accel_loss', torch.tensor(0.0)).item()
                yaw_val = loss_dict.get('yaw_loss', torch.tensor(0.0)).item()
                dist_val = loss_dict.get('dist_loss', torch.tensor(0.0)).item()
...
def main():
    ...
    #
    # <--- 변경된 부분: GNN-Transformer 모델 초기화
    #
    Logger.log('Initializing GNN-Transformer Model...')
    model = FITTrafficModel(cfg.past_len, cfg.future_len, cfg.map_obs_size_pix, len(train_dataset.categories),
...
#------------------------------Loss Added (사용자 코드 유지)------------------------------    

    print(cfg.loss_accel)
    print(cfg.loss_yaw)  
    print(cfg.loss_dist)
...
#------------------------------Loss Added (사용자 코드 유지)------------------------------    
    ...
    # run training
    ckpts_path = os.path.join(cfg.out, 'checkpoints_boston')
    mkdir(ckpts_path)
    step_counter = 0
    min_eval_loss = ckpt_eval_loss
    
    # (사용자가 제공한 Matplotlib 학습 루프를 그대로 사용합니다)
    ...
    for epoch in range(ckpt_epoch, cfg.epochs):
        ...
        plt.savefig('loss.jpg', format='jpeg')
        # ----------------------------------------------------------
        ...
        if epoch % cfg.val_every == 0:
            ...
            with torch.no_grad():
                ...
                if mean_eval_loss < min_eval_loss:
                    ...
                    save_file = os.path.join(ckpts_path, 'best_eval_model_transCVAE.pth')
        ...
        if epoch % cfg.save_every == 0:
            ...
            save_file = os.path.join(ckpts_path, 'epoch_%08d_model_transCVAE.pth' % (epoch))
            save_state(save_file, model, optimizer, cur_epoch=epoch, min_val_loss=min_eval_loss)
            save_file = os.path.join(ckpts_path, 'latest_model_transCVAE.pth')
            save_state(save_file, model, optimizer, cur_epoch=epoch, min_val_loss=min_eval_loss)
            if use_wandb:
                wandb.save(save_file)
