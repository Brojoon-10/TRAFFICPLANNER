...
class FITTrafficModel(nn.Module):
    def __init__(self, npast, nfuture, map_obs_size_pix, nclasses,
        ...
        ###########################################################################
        # HJ_ADDED: Agent Cross-Attention for Future Interaction (v2 - trajectory history)
        ###########################################################################
        # This module enables agents to see other agents' TRAJECTORY HISTORY during
        # future trajectory decoding (autoregressive step).
        #
        # - Query: Current agent's GRU hidden state
        # - Key/Value: Other agents' GRU hidden states (contains trajectory history!)
        #
        # IMPROVEMENT over v1:
        # - v1: Only used current position (x,y,hx,hy) -> no velocity/momentum info
        # - v2: Uses GRU hidden state -> contains Past + Future steps so far
        #       This allows Ego to see Sur's movement pattern, speed, and direction
        ###########################################################################
        # Project GRU hidden state for Agent Attention (D -> D, learnable transformation)
        self.agent_hidden_encoder = MLP([self.gcn_hidden_dim, 64, self.gcn_hidden_dim])
    ...
    def autoregressive_decoder(self, scene_graph, map_feat, past_seq_out, z, map_idx, map_env,
        ...
        for t in range(FT):
            ...
            # HJ_ADDED: Agent Cross-Attention for Future Interaction (v2 - trajectory history)
            ###########################################################################
            # Compute attention over other agents' GRU hidden states.
            # This enables each agent to "see" other agents' TRAJECTORY HISTORY:
            # - Past 4 steps (encoded in initial hidden state)
            # - Future steps predicted so far (accumulated by GRU)
            #
            # For each agent i:
            #   Query: agent i's GRU hidden state
            #   Key/Value: all agents' GRU hidden states (trajectory history!)
            #
            # IMPROVEMENT over v1 (position-only):
            # - v1: Only saw current position -> no velocity/momentum info
            # - v2: Sees trajectory history -> movement pattern, speed, direction
            ###########################################################################

            # Get all agents' GRU hidden states (contains trajectory history)
            # cur_mem_state: (num_layers, bsize, D) -> use last layer
            all_agents_hidden = cur_mem_state[-1]  # (bsize, D)

            # Project hidden state for attention (learnable transformation)
            all_agents_feat = self.agent_hidden_encoder(all_agents_hidden)  # (bsize, D)
            ...
            if t == 0 and hasattr(self, '_debug_attention') and self._debug_attention:
                print(f"\n=== AGENT ATTENTION DEBUG (v2 - trajectory history) t={t} ===")
                ...
                for agent_idx in range(min(5, bsize)):
                    ...
                    self_attn = agent_w[agent_idx, self_idx].item()
