...
def run_one_epoch(data_loader, model, map_env, loss_fn, device, out_path,
    ...
    if test_recon_viz_multi:
        recon_multi_agent_out_path = os.path.join(out_path, 'viz_recon_multi_transCVAE_fine_tuned')
    ...
    if test_sample_viz_rollout:
        sample_rollout_out_path = os.path.join(out_path, 'viz_sample_rollout_transCVAE_fine_tuned')
...
def main():
    ...
    # device setup
    device = torch.device('cuda:0')#initially get_device()
