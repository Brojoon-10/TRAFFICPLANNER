...
#     # print(ego_agent_dists)
#     # print((ego_agent_dists < feasibility_thresh).sum(dim=[1, 2]))
#     # print(feasible)
...
def determine_feasibility_nusc(samples, normalizer, feasibility_thresh, 
    ...
    # [DEBUG] Print statement to indicate the start of debugging
    print(f"\n{'='*20} [DEBUG] Feasibility Check Start (Scene with {NA} agents) {'='*20}")
    ...
    if feasibility_infront_min is not None:
        ...
        ego2agent = agent_pos - ego_pos
        # [DEBUG] Prevent division by zero error
        ego2agent_norm = torch.norm(ego2agent, dim=-1, keepdim=True)
        ego2agent = torch.where(ego2agent_norm > 1e-6, ego2agent / ego2agent_norm, torch.zeros_like(ego2agent))
        
        cossim = torch.sum(ego2agent * ego_h, dim=-1) # (NA-1, NS, T')
        infront = cossim >= feasibility_infront_min

        # [DEBUG] Track agents failing the 'in-front' filter
        for agent_idx in range(NA):
            # If the 'infront' condition is never True for any sample or timestep
            if not infront[agent_idx].any():
                print(f"[DEBUG] Agent {agent_idx + 1}: FAILED 'in-front' check (always behind ego).")
    ...
    # [DEBUG] Step 1: Track results for the distance filter
    feasible = (ego_agent_dists < feasibility_thresh).sum(dim=[1, 2]) > 0
    print(f"\n[DEBUG] Step 1: Initial check based on distance (< {feasibility_thresh}m)")
    for agent_idx in range(NA):
        if feasible[agent_idx]:
            print(f"  -> Agent {agent_idx + 1}: PASSED")
        else:
            min_dist_val = torch.min(ego_agent_dists[agent_idx]).item()
            print(f"  -> Agent {agent_idx + 1}: FAILED (Closest distance: {min_dist_val:.2f}m)")

    # [DEBUG] Step 2: Track results for the non-drivable area filter
    if check_non_drivable_separation:
        feasible_before = feasible.clone()
        min_samp_inds = min_samp_inds[torch.arange(NA), feasible_time_step-feasibility_time]
        min_samp_agent_trajs = agent_samples[torch.arange(NA), min_samp_inds]
        min_samp_ego_trajs = ego_samples.expand(NA, NS, FT, 4)[torch.arange(NA), min_samp_inds]
        min_samp_agent_state = min_samp_agent_trajs[torch.arange(NA), feasible_time_step][:, :2]
        ...
        print(f"\n[DEBUG] Step 2: Check for non-drivable area separation")
        for agent_idx in range(NA):
            # Case where it was True before but is now False
            if feasible_before[agent_idx] and not feasible[agent_idx]:
                print(f"  -> Agent {agent_idx + 1}: FAILED (Separated by non-drivable area)")
            elif not feasible_before[agent_idx]:
                 print(f"  -> Agent {agent_idx + 1}: SKIPPED (Already failed previous check)")
            else:
                print(f"  -> Agent {agent_idx + 1}: PASSED")

    # [DEBUG] Step 3: Track results for the velocity filter
    feasible_before = feasible.clone()
    agent_vels = torch.norm(agent_samples[:, :, 1:, :2] - agent_samples[:, :, :-1, :2], dim=-1) # (NA-1, NS, FT-1)
    max_vels = torch.max(torch.max(agent_vels, dim=1)[0], dim=1)[0] # (NA-1, )
    feasible = torch.logical_and(feasible, max_vels > feasibility_vel)
    
    print(f"\n[DEBUG] Step 3: Check for minimum velocity (> {feasibility_vel} m/step)")
    for agent_idx in range(NA):
        # Case where it was True before but is now False
        if feasible_before[agent_idx] and not feasible[agent_idx]:
            agent_max_vel = max_vels[agent_idx].item()
            print(f"  -> Agent {agent_idx + 1}: FAILED (Max velocity: {agent_max_vel:.2f} m/step)")
        elif not feasible_before[agent_idx]:
            print(f"  -> Agent {agent_idx + 1}: SKIPPED (Already failed previous check)")
        else:
            print(f"  -> Agent {agent_idx + 1}: PASSED")
            
    # [DEBUG] Final result (before override)
    print(f"\n[DEBUG] Original Feasibility Result: {feasible.cpu().numpy()}")

    # ========================= [MODIFIED] Force Output to True =========================
    # Override the final 'feasible' tensor to be all True for every agent.
    # This ensures that every agent proceeds to the next stage, regardless of the checks above.
    feasible = torch.ones(NA, dtype=torch.bool, device=samples.device)
    
    # Since we are forcing feasibility, the original time and distance values might be
    # inconsistent (e.g., distance could be 'inf'). We provide safe, default values
    # to prevent errors in the main script that uses these return values.
    feasible_time_step.fill_(feasibility_time) # Fill with the initial time threshold
    feasible_dist.fill_(0.0)                   # Fill with zero
    
    print(f"[DEBUG] OVERRIDDEN Final Feasibility Result: {feasible.cpu().numpy()}")
    # ===================================================================================

    print(f"{'='*25} [DEBUG] Check End {'='*25}")
...
def viz_optim_results(out_path, scene_graph, map_idx, map_env,
                        ...
                        # viz_bounds=[-100.0, -100.0, 100.0, 100.0],
                        # viz_bounds=[-65.0, -65.0, 65.0, 65.0],
                        # viz_bounds=[-60.0, -60.0, 80.0, 80.0],
                        # viz_bounds=[-17.0, -38.5, 65.0, 35.5],
                        # viz_bounds=[-17.0, -38.5, 100.0, 35.5],
                        # viz_bounds=[-34.0, -36.0, 40.0, 36.0],
