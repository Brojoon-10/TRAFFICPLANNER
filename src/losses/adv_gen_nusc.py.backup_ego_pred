    ...
    def forward(self, future_pred, tgt_traj, z, prior_out,
        ...
        if return_mins:
            if cur_min_agt is not None:
                loss_out['min_agt'] = np.array(cur_min_agt, dtype=np.int32)
            if cur_min_t is not None:
                loss_out['min_t'] = np.array(cur_min_t, dtype=np.int32)
...
VEH_COLL_THRESH = 0.01 # HJ_EDITED: lowered from 0.02 to 0.01 to catch near-miss collisions
...
def check_single_veh_coll(traj_tgt, lw_tgt, traj_others, lw_others):
    ...
    coll_time = np.ones((NA), dtype=np.int32)*FT
