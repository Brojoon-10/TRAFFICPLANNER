...
def run_one_epoch(data_loader, batch_size, model, map_env, device, out_path, loss_weights,
    ...
    out_list = []
    out_traj = torch.tensor(out_list)
    ...
    for i, data in enumerate(pbar_data):
        ...
        try:
            ...
            if feasible is None:
                Logger.log('Only ego vehicle in scene.')
            elif torch.sum(feasible).item() == 0:
                Logger.log('Infeasible, no vehicles near ego.')
...
#-------------------------------------------------------Opt---------------------------------------------------
            ...
            final_result_traj = init_future_pred.clone().detach()


            final_decoder_out = {}
            final_decoder_out['future_pred'] = final_result_traj  # or init_future_pred

            attack_agt = torch.zeros((B,), dtype=torch.long, device=device)
            attack_t   = torch.zeros((B,), dtype=torch.long, device=device)
            adv_succeeded = [False]*B
            sol_succeeded = [False]*B

            Logger.log('Skipped adv & solution optim. Using init_future_pred as final_result_traj!')
...
#-------------------------------------------Multi------------------------------------------
            ...
            out_list.append(init_traj)
            out_traj = torch.stack(out_list,1)
...
#-------------------------------------------Multi------------------------------------------
    ...
    viz_optim_results(pred_out_path, scene_graph, map_idx, map_env, model,
                    # init_future_pred, planner_name, cur_attack_agt,
                    out_traj, planner_name, cur_attack_agt,
                    cur_crop_t,
                    bidx=b,
                    show_gt=False, # show entire nuscenes scene
                    ow_gt=out_traj)
