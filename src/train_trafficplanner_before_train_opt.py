...
def parse_cfg():
    ...
    parser.add_argument('--freeze_z_global', action='store_true',
    ...
    parser.add_argument('--use_potential_loss', action='store_true', default=True,
    ...
    parser.add_argument('--use_sparse_loss', action='store_true', default=False,
    ...
    parser.add_argument('--tf_segment_len', type=int, default=3,
                        help='Number of steps per segment before resetting to GT')
    ...
    parser.add_argument('--use_lane_lines', action='store_true', default=True,
...
def main():
    ...
    print(f'tf_segment_len: {cfg.tf_segment_len}')
    ...
    for epoch in range(ckpt_epoch, cfg.epochs):
        ...
        step_counter, _ = run_one_epoch(train_loader, model, map_env, loss_fn, device, cfg.out,
                                        ...
                                        tf_segment_len=cfg.tf_segment_len)
        train_loss.append(_)
        ax1.clear()
        ax1.plot(train_loss)
        ax1.set_title(f"Train Loss (Phase {cfg.phase})")
        plt.savefig('loss_trafficplanner.jpg', format='jpeg')
