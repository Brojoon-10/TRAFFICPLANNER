...
# from models.fit_traffic_model_trans import FITTrafficModel
# from models.fit_traffic_model import FITTrafficModel
# from models.traffic_model_trans_decoder_changed import FITTrafficModel

# NEW: Agent Cross-Attention with trajectory history (v2)
from models.transCVAE_agent_attention import FITTrafficModel
...
from utils.adv_gen_optim import run_adv_gen_optim, run_adv_gen_optim_reactive, run_adv_gen_optim_agent_attention, compute_adv_gen_success
...
def parse_cfg():
    ...
    # HJ_EDITED: Added ego_pred_reactive and ego_pred_agent_attention planner options
    parser.add_argument('--planner', type=str, default='ego',
                        choices=['ego', 'hardcode', 'ego_pred', 'ego_pred_reactive', 'ego_pred_agent_attention'],
                        help='Which planner to attack. ego=GT replay, hardcode=rule-based reactive, ego_pred=model prediction (no reaction), ego_pred_reactive=sliding window reaction, ego_pred_agent_attention=Agent Cross-Attention based hardcode-style.')
...
def run_one_epoch(data_loader, batch_size, model, map_env, device, out_path, loss_weights,
    ...
    for i, data in enumerate(pbar_data):
        ...
        # save_root = "./src/scene_graph/test2"     
        # os.makedirs(save_root, exist_ok=True)
        # torch.save(scene_graph,       
        #            os.path.join(save_root, f"scene_{i:06d}.pt"))
        if empty_cache:
            empty_cache = False
            gc.collect()
...
#-----------------------------------------HJ ADDED-------------------------------------------
            ...
            # if planner_name == 'ego':
            if planner_name in ['ego', 'ego_pred', 'ego_pred_reactive', 'ego_pred_agent_attention']: # HJ ADDED
...
#-----------------------------------------HJ ADDED-------------------------------------------
            ...
            if planner_name == 'hardcode':
                ...
                if np.sum(bvalid) < B:
                    ...
                    avalid = np.zeros((NA), dtype=np.bool_) # which agents are part of new graphs
                    for b in range(B):
                        if bvalid[b]:
            ...
            if planner_name == 'hardcode':
                ...
                if viz:
                    ...
                    cur_seq_str = f"sample_{'_'.join(f'{b + START_NUM:03d}' for b in cur_batch_i)}"
            ...
            ############### HJ_EDITED: Support ego_pred_reactive and ego_pred_agent_attention planners ###############
            if planner_name == 'ego_pred_reactive':
                # Sliding window approach for reactive ego prediction
                num_iters_per_round = max(1, num_iters // model.FT)
                adv_gen_out = run_adv_gen_optim_reactive(cur_z, lr, loss_weights, model, scene_graph, map_env, map_idx,
                                                         num_iters_per_round, embed_info,
                                                         tgt_prior_distrib, other_prior_distrib,
                                                         feasibility_time, feasibility_infront_min)
            elif planner_name == 'ego_pred_agent_attention':
                # Agent Cross-Attention based hardcode-style optimization
                # Ego reacts via model's internal Agent Attention (no external planner)
                adv_gen_out = run_adv_gen_optim_agent_attention(cur_z, lr, loss_weights, model, scene_graph, map_env, map_idx,
            ...
            for b in range(B):
                ...
                if save:
                    ...
                    fout_path = os.path.join(cur_scene_out_path, 'scene_%04d.json' % (cur_batch_i[b]+START_NUM))
                ...
                if viz:
                    ...
                    pred_prefix = 'test_sample_%d_before' % (cur_batch_i[b]+START_NUM)
...
#-------------------------------------------Sur GT or Pred----------------------------------------------------     
                    ...
                    viz_optim_results(pred_out_path, scene_graph, map_idx, map_env, model,
                                        ...
                                        init_traj, planner_name, cur_attack_agt,
                                        cur_crop_t,
                                        bidx=b,
...
#-------------------------------------------Sur GT or Pred----------------------------------------------------                    
                    ...
                    pred_prefix = 'test_sample_%d_after' % (cur_batch_i[b]+START_NUM)
                    pred_out_path = os.path.join(cur_viz_out_path, pred_prefix)
                    viz_optim_results(pred_out_path, scene_graph, map_idx, map_env, model,
                    ...
                    if adv_succeeded[b]:
                        pred_prefix = 'test_sample_%d_sol' % (cur_batch_i[b]+START_NUM)
                        pred_out_path = os.path.join(cur_viz_out_path, pred_prefix)
...
def main():
    ...
    ############### MODIFIED: Only load lanegraph for hardcode planner ###############
    # HJ_EDITED: ego_pred and ego_pred_reactive don't need lanegraph either
    map_env = FITMapEnv(data_path,
                        bounds=cfg.map_obs_bounds,
                        L=cfg.map_obs_size_pix,
                        W=cfg.map_obs_size_pix,
                        layers=cfg.map_layers,
                        device=device,
                        load_lanegraph=(cfg.planner == 'hardcode'),  # Only needed for hardcode planner
                        )
    ####################################################################################

    test_dataset = FITDataset(data_path, map_env,
                            split=cfg.split, # val # adv
    ...
    # create model
    # model = FITTrafficModel(cfg.past_len, cfg.future_len, cfg.map_obs_size_pix, len(test_dataset.categories),
    ...
    Logger.log('Initializing GNN-Transformer Model...')
    model = FITTrafficModel(cfg.past_len, cfg.future_len, cfg.map_obs_size_pix, len(test_dataset.categories),
                     map_feat_size=cfg.map_feat_size,
                     gcn_hidden_dim=cfg.gcn_hidden_dim, # <-- 신규
                     latent_size=cfg.latent_size,
                     output_bicycle=cfg.model_output_bicycle,
                     dt = cfg.dt,
                     # GNN-Transformer 인자
                     step_feat_dim=cfg.step_feat_dim, # <-- 신규
                     gcn_message_dim=cfg.gcn_message_dim, # <-- 신규
                     transformer_nhead=cfg.transformer_nhead, # <-- 신규
                     transformer_nlayer=cfg.transformer_nlayer, # <-- 신규
                     # Decoder 인자
                     decoder_gru_layers=cfg.decoder_gru_layers, # <-- 신규
                     decoder_attn_heads=cfg.decoder_attn_heads, # <-- 신규
                     # Map Conv 인자 (기존과 동일)
                     conv_channel_in=map_env.num_layers,
                     conv_kernel_list=cfg.conv_kernel_list,
                     conv_stride_list=cfg.conv_stride_list,
