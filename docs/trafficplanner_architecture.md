...
- **이중 잠재변수**: `z_global` (64dim, pretrain 후 frozen) + `z_local` (64dim, 매 스텝 ego 전용)
...
├── latent_prior_net(MLP: 130→128→128) → split → z_prior_mean(64), z_prior_var(64)
...
└── latent_posterior_net(MLP: 194→128→128) → z_post_mean(64), z_post_var(64)

Step 4: z_global 샘플링
└── z = rsample(z_post_mean, z_post_var) → (NA, 64)
...
| Sur Decoder | `decoder_net` | 196→64→2 | z(64)+past_feat(64)+map(64)+sem(2)+lw(2) |
...
│   MLP: 130 → 128 → 128     MLP: 194 → 128 → 128                   │
│   (64+64+2)                 (64+64+64+2)                             │
│        ↓                          ↓                                  │
│   z_prior (μ:64, σ²:64)    z_post (μ:64, σ²:64)                   │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `latent_prior_net` | MLP | 130→128→128 | z_global prior | Frozen |
| `latent_posterior_net` | MLP | 194→128→128 | z_global posterior | Frozen |
| `map_conv` | CNN | 4→128, 6-layer | map encoding | Frozen |
| `map_feature` | Linear | ?→64 | map feat projection | Frozen |
| `decoder_net` | IndivGCN | 196→64→2 | sur trajectory | Frozen |
...
| `z_combine_mlp` | MLP | 128→64→64 | z_global+z_local merge | **Trainable** |
...
| 11 | z_global 확대 | latent_size 32→64, z_local(64)과 균형 맞춤 |
